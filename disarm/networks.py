...
class DiscreteVAE(tf.keras.Model):
  ...
  def get_layer_grad_estimation(
    ...
    if grad_type == 'ar':
    ...
    elif grad_type == 'ind-disarm':
      u_noise = _sample_uniform_variables(
          sample_shape=tf.shape(encoder_logits),
          nfold=1)
      u_noise2 = _sample_uniform_variables(
          sample_shape=tf.shape(encoder_logits),
          nfold=1)
      sigma_abs_phi = tf.math.sigmoid(tf.math.abs(encoder_logits))
      b1 = tf.cast(u_noise < sigma_phi, tf.float32)
      b2 = tf.cast(u_noise2 < sigma_phi, tf.float32)
      f1 = self.get_elbo(input_tensor, b1)[:, tf.newaxis]
      f2 = self.get_elbo(input_tensor, b2)[:, tf.newaxis]
      # the factor is I(b1+b2=1) * (-1)**b2 * sigma(|phi|)
      disarm_factor = ((1. - b1) * (b2) + b1 * (1. - b2)) * (-1.)**b2
      ...
      z_noise1 = tf.cast(u_noise1 > 0.5, tf.float32)

      u_noise = _sample_uniform_variables(sample_shape=tf.shape(encoder_logits),nfold=1)
      
      b1 = tf.cast(u_noise < (1.- ((-1.)**z_noise1)*epsilon)/2., tf.float32)
      b2 = tf.cast(1. - u_noise < (1.-((-1.)**z_noise1)*epsilon)/2., tf.float32)
      disarm_factor1 = ((1. - b1) * (b2) + b1 * (1. - b2)) #indicator for diff
      disarm_factor2 = b2*b1  + (1. -b2)*(1. -b1) #indicator for same
      ...
      minus_sigma_abs_phi = 1. - tf.math.sigmoid(tf.math.abs(encoder_logits))
      log_minus_sigma_abs_phi = safe_log_prob(minus_sigma_abs_phi)
      ot = safe_log_prob(1 - 2.*minus_sigma_abs_phi)
      mask = tf.cast(sigma_phi < 0.5, tf.float32)

      disarm_factor = disarm_factor1*log_minus_sigma_abs_phi + disarm_factor2*ot - disarm_factor1*safe_log_prob((1.-epsilon)/2) - disarm_factor2*safe_log_prob(epsilon/2.)


      ldisarm_factor = tf.math.reduce_sum(disarm_factor, axis = 1, keepdims = True) - disarm_factor
      
      layer_grad = ((-1.)**(b2))*(f1 - f2)*tf.math.exp(ldisarm_factor)*sigma_phi*(1.0 - sigma_phi)*(1. - mask*b1*b2)*(1. - (1. - mask)*(1.-b1)*(1.-b2)) #last term 0s out cases where (1,1) sampled but theta < 0.5 and vv
    
    elif grad_type == 'is-disarm-alt':
      u_noise = _sample_uniform_variables(sample_shape=tf.shape(encoder_logits),nfold=1)
      minus_sigma_abs_phi = 1. - tf.math.sigmoid(tf.math.abs(encoder_logits))
      b1 = tf.cast(u_noise > 0.5, tf.float32)
      b2 = tf.cast(u_noise < 0.5, tf.float32)
      f1 = self.get_elbo(input_tensor, b1)[:, tf.newaxis]
      f2 = self.get_elbo(input_tensor, b2)[:, tf.newaxis]
      
      log_sigma_phi = tf.math.log(tf.clip_by_value(sigma_phi, 1e-14, 1.0))
      log_1msigma_phi = tf.math.log(tf.clip_by_value(1.0 - sigma_phi, 1e-14, 1.0))
      k = log_sigma_phi.shape[1] 
      loglikp1 = b1*log_sigma_phi + (1.0-b1)*log_1msigma_phi
      loglikp2 = b2*log_sigma_phi + (1.0-b2)*log_1msigma_phi
      isdisarm_log_factors1 = tf.math.reduce_sum(loglikp1, axis = 1, keepdims = True) - loglikp1 + (k-1)*tf.math.log(2.0)
      isdisarm_log_factors2 = tf.math.reduce_sum(loglikp2, axis = 1, keepdims = True) - loglikp2 + (k-1)*tf.math.log(2.0)
      layer_grad = ((-1.)**(b2))*(f1*tf.math.exp(isdisarm_log_factors1) - f2*tf.math.exp(isdisarm_log_factors2))*sigma_phi*(1.0 - sigma_phi)

    
    elif grad_type == 'bitflip':
      u_noise = _sample_uniform_variables(
          sample_shape=tf.shape(encoder_logits),
          nfold=1)
      ...
      u_noise2 = _sample_uniform_variables(
          sample_shape=tf.shape(encoder_logits),
          nfold=1)
      u_noise3 = _sample_uniform_variables(
          sample_shape=tf.shape(encoder_logits),
          nfold=1)
      sigma_abs_phi = tf.math.sigmoid(tf.math.abs(encoder_logits))
      b1 = tf.cast(u_noise > 1. - sigma_phi, tf.float32)
      b2 = tf.cast(u_noise2 > 1. - sigma_phi, tf.float32)
      b3 = tf.cast(u_noise3 < 0.01, tf.float32)
      b1 = b1*(1.-b3) + (1.-b1)*b3 + b3*b1
      b2 = (1.-b3)*b2 
      f1 = self.get_elbo(input_tensor, b1)[:, tf.newaxis]
      f2 = self.get_elbo(input_tensor, b2)[:, tf.newaxis]
      # the factor is I(b1+b2=1) * (-1)**b2 * sigma(|phi|)
